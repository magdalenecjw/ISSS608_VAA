...
```{r loading}
pacman::p_load(patchwork, tidyverse, ggstatsplot, ggdist, plotly, knitr, nortest)
...
Likewise, there are too many storey range categories as the storeys were coded in groups of 3 storeys. We will recode this variable into groups of 9 storeys up until 27 storey, before grouping them as 28 storey and higher (since most HDB apartment blocks are [built with 30 floors and less](https://www.propertyguru.com.sg/property-guides/tallest-hdb-flats-50281)).
...
hdb_2022 <- hdb_data_cleaned %>%
  ...
  select("town", "region", "flat_type", "storey_recoded", "floor_area_sqm", 
         "remaining_lease", "resale_price", "trans_date")
```
